...
Net: this revision used to call ``Base.metadata.create_all()``, which is a
snapshot of whatever ``models.py`` said on the day it ran, not a diff. That is
why production and the repo drifted apart in 11 places without a single error
anywhere -- an edit to a ``__table_args__`` simply never reached a database that
``create_all`` had already "handled". The full divergence list, with which side
was right in each case, is
``docs/operations/production-database-schema-drift.md``.

The DDL below is generated from the same metadata ``create_all`` was reading, so
it builds a byte-identical schema -- proved by diffing ``pg_catalog`` after
``alembic upgrade head`` against the snapshot taken before the rewrite. Nothing
about a live database changes: production is stamped well past this revision and
will never run it again. What changes is that the migration history can now
*describe* the schema, so ``alembic revision --autogenerate`` produces a real
diff and ``scripts/check_schema_drift.py`` can fail a PR that lets the two drift
apart again.

Two guards elsewhere stay load-bearing, deliberately. ``0002_notification_event``
and ``0008_legislator_election_history`` skip themselves when their table already
exists, because this baseline still creates every table ``models.py`` declares --
the same set ``create_all`` created. Removing a table from this revision so a
later one can create it is a behaviour change, not a transcription, so it is not
folded in here.

Three indexes are created below by hand rather than from the metadata, because
``models.py`` does not declare them and cannot: ``0012`` later swaps the ivfflat
vector index for an HNSW one, and a model can only declare one of the two. They
are listed in the ignore set in ``scripts/check_schema_drift.py`` for the same
reason.
"""
...
# Every table this revision creates, for the downgrade to remove. Listed
# explicitly rather than read back from the metadata: a revision that imports
# models.py describes whatever that file says today, not what it created, which
# is the whole defect #100 exists to remove.
TABLES = (
    "ingestion_run",
    "jurisdiction",
    "user_account",
...
# Enum types are created implicitly by the ``create_table`` calls that reference
# them, but ``drop_table`` does not remove them. Dropping them explicitly is what
# makes an upgrade -> downgrade -> upgrade round trip work; without it the second
...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        "ai_enrichment",
        ["bill_id", "enrichment_type"],
        unique=True,
        postgresql_where=sa.text(
            "bill_id IS NOT NULL AND enrichment_type = 'bill_summary'::enrichment_type AND is_current = true"
        ),
    )
    op.create_table(
        "bill_version_section",
        ...
        sa.UniqueConstraint(
            "bill_version_id",
            "section_id_text",
            name=op.f("uq_bill_version_section_bill_version_id_section_id_text"),
        ),
    )
    op.create_index(
    ...
    # Not declared in models.py, so not in the generated DDL above. The ivfflat
    # index is deliberately the original one: 0012 replaces it with HNSW, and
    # this revision has to leave the schema in the state 0012 expects to find.
    op.create_index(
...
def downgrade() -> None:
    """Take the schema back to empty.

    Every drop is ``IF EXISTS``, and every table drops with ``CASCADE``, because
    that is what ``metadata.drop_all()`` did before this revision was written out
    by hand and several later revisions quietly rely on it. 0002, 0003 and 0008
    each drop an object this revision also creates, and Alembic runs their
    downgrades first -- so a strict drop here fails on an object that is already
    gone. ``CASCADE`` also removes the foreign keys for us, which is why no
    dependency order is needed below.
    """
    for index_name in (
    ...
    # Dropping a table does not drop the enum types its columns referenced, and a
    # second upgrade then fails with "type already exists".
