...
def _assert_cite_or_refuse(answer: dict, kind: str) -> None:
    ...
    if kind == "topic_bills":
        ...
        if answer["total_matches"] == 0:
            ...
            return
        assert items, "a non-zero match count must render at least one citation"
        # Per docs/product-and-onboarding/grounded-ask-spec.md §4.2 (topic_legislators), the citation
...
def test_bill_text_answer_cites_the_resolved_bill(client, monkeypatch):
    """Scenario 1 (docs/product-and-onboarding/grounded-ask-spec.md §4.1 / §9.4, bill_text): a question
...
def test_bill_text_resolves_a_bill_by_fuzzy_title(client, monkeypatch):
    """Scenario 1 (docs/product-and-onboarding/grounded-ask-spec.md §4.1, bill_text): a question with no
...
def test_vote_deflection_resolves_named_bill_and_degrades_otherwise(
    ...
    """Scenario 4 v1 (docs/product-and-onboarding/grounded-ask-spec.md §4.5 / §9.4, Vote deflection): a
