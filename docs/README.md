...
- [Page metadata for search and sharing — decisions](architecture/page-metadata-for-search-and-sharing-decisions.md) — why every address serves the same title today, the wording rule for each page type, and the four ways to fix it with the recommended one costed (proposal, not built)
