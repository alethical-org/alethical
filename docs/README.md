...
- [How the Campaign money section works (plain English)](product-onboarding/campaign-money-section-guide.md) — the public /money landing and its lanes (built and plainly-not-built), the reports shelf in its nothing-published state, and what a signed report's page will carry when one publishes
...
- [Published writing — decisions](architecture/published-writing-decisions.md) — what Alethical publishes in its own name and the settled calls governing it: nested addresses and what flat lost on, a standalone teaching piece, an emptied set, who authors a term link, plus the open kind-word question and the 4 fields no piece carries yet
