...
- The deflection paragraph (§9.1 wording). The old constraint here — *don't strengthen this to "every member, every vote" unless the Votes tab renders member-level breakdowns in-app* — is **satisfied as of Jul 21, 2026**: the Votes tab renders the party-grouped per-member grid on web ([#431](https://github.com/alethical-org/alethical/pull/431)) and mobile ([#451](https://github.com/alethical-org/alethical/pull/451)), consuming `VoteEvent.votes` (`apps/frontend/src/components/billDetail/VotesTab.tsx`). The stronger wording is therefore *permitted* by `.claude/rules/grounded-answers.md` rule 6 (copy claims match shipped capability) — whether to adopt it is an open copy decision, not a blocked one.
