...
- Exact-commit checks passed: 2,805 backend tests and 3,095 frontend tests. Production build passes the unchanged first-download size limit. Added focused coverage for delayed scroll callbacks and waiting for returned names before restoring position; native-click/reload/return preserved 736.5px as rounded 737px. The earlier 377px observation was browser-test click auto-scrolling before navigation, not a product return defect.
- Remaining: final frontend checks, [pull request 2239](https://github.com/alethical-org/alethical/pull/2239), merge queue, deployment and live browser checks. Test server handles: frontend local port18743, backend18744, task-owned disposable database; stop after verification.
