...
- **`sitemap.xml`** (`https://www.alethical.com/sitemap.xml`) lists every bill, every legislator, and every real numbered Bills and Legislators directory page. Records carry the date they really last changed. It is built when asked for and then cached, so a newly ingested bill appears without waiting for a release.
