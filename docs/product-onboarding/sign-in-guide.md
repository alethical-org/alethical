...
**Add a password** or **Change password**, **Email preferences**, an **Admin** group for
approved administrators, and **Sign out**. The password row keeps its method-specific
wording and neutral **Password** fallback. People without administrator access do not
see the Admin group.

[How the top bar works § Inside the account menu](site-navigation-guide.md#inside-the-account-menu)
owns the menu's pictures, labels, count, spacing, keyboard feedback, short-screen scrolling,
and sign-out progress and retry behavior. Neither menu shows a note about staying signed
in on other devices before, during, or after a failed sign-out.
