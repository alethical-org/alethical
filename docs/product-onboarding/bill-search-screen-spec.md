...
- **URL-addressable filters** — tracked in [#135](https://github.com/alethical-org/alethical/issues/135), split by milestone to keep v1 lean:
  ...
  - **Shipped (full serialization):** every filter serialises *out* to the URL
    (`/bills?q=&scope=&chamber=&status=&issue=&omnibus=&session=&sort=&page=`; old shared
    `topic` links are read as Issue links) so reload, share,
...
7. **Pagination** — Previous · "Page N of M" · Next (server-backed `limit`/`offset`,
   ...
   `Pagination` control, so it fires on Previous and Next only — never on a
   filter, sort, or search keystroke. Unfiltered directory controls are ordinary
   web links; filtered search stays app-only. Any new paged list inherits it the
   same way.
...
| Session / year | dropdown defaults to the current Legislature across regular + special sessions, with every individual ingested session available newest first, plus two inert greyed-out prior bienniums. Individual sessions keep the existing labels; the combined choice reads "2025–26 Legislature". A special-session result card carries its own session label because its bill numbers start over ([#746](https://github.com/alethical-org/alethical/issues/746)). | default `scope=legislature`, or `session` after the reader narrows it |
| Omnibus | toggle "Omnibus only" | `omnibus` |
| Issue | selectable pills **with live bill counts** ("Education 214"); Ask uses this same filter and label | `policy_area` (counts from `GET /policy-areas`) |

The 3 entry paths stay distinct only where their meaning is distinct:
