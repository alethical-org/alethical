...
- **Session** — Search starts with the whole 2025–26 Legislature, including regular
  and special sessions. You can narrow it to one session. Two sessions are loaded:
  the 2025–26 regular session, and
...
Ask and the issue chips now share one Issue filter. Search starts with the whole
Legislature, so all three paths include regular and special sessions unless you pick
one session yourself.
...
The hidden issue labels are produced when Alethical analyzes the official bill title,
description, and bill-text excerpts. A fixed public vocabulary then groups related raw
labels: for example, `property tax`, `income tax`, and `sales and use tax` all appear
under **Taxation**. These are Alethical classifications, not official Legislature labels.
...
Tap a card to open the full bill; results are split into pages with Previous / Next on the
plain Bills list.
