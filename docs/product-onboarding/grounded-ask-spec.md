...
- **No-matches (current behavior):** an in-scope topic with zero (or below-threshold) matching bills gets the **NO MATCHES** response — "No current-session bills match *{topic}*" + Search link + the hero chips. This is a distinct state from out-of-scope refusal (§9.1): in scope, just empty. A zero-result answer has nothing to cite, so it must never render as a normal answer.
...
- [ ] Sign-in from an answer page returns to that exact answer with the composer unlocked, never a generic dashboard (§9.2). Tracking is a roadmap preview — the Track button is an inert dashed control everywhere it appears (no sign-in, no state change, no "✓ Tracking" state); the affirmed-Track return path is deferred with the tracking feature.
...
2. **Answer persistence — decided for today, open beyond it.** Today: Copy link/Share emit `?q=` re-run links (topic answers are deterministic DB queries, so re-runs are stable where it matters; `bill_text` prose may vary slightly — acceptable today). Later ([#82](https://github.com/alethical-org/alethical/issues/82)): persist answer snapshots with a public id — the shared URL then shows exactly what the asker saw, and the snapshot log doubles as an audit trail for QA-ing the guardrail. Signed-out sessions still don't persist; sign-in prompt covers save/follow-up.
...
| `topic_bills` (`bills-list`) | Fixed intro line with the **matched topic rendered as a highlighted pill** (makes the router's interpretation visible — keep it); ≤6 bill cards (bill pill · title · status badge · AI summary · View bill → · Track [roadmap preview, inert — §9.2]), ordered per §4.2; "See all N {topic} bills in Search →" overflow; Copy link/Share | None — **each bill card is its own citation**; a Sources rail would duplicate the list. |
...
- **Rail bill card.** When the rail shows a **single** bill (`bill_text`'s answering bill, the deflection's resolved bill): bill pill · title · chamber · status · **chief author** ("Chief author: {name} ({party}–{district})", linking to the legislator profile; from `Sponsorship` rows with role `chief_author`) · **signed / last-action date** (from `BillAction`) · a quiet **"See votes →"** link to the bill's Votes tab (the §9.3 `?tab=votes` anchor) · Track button. Rationale: the card puts the product's three promises one click from every answer — what it says (the answer), who made it (author), how everyone voted (votes link). Keep it a card, not a dashboard: those seven elements, nothing more. When a rail lists **multiple** bills (`legislators`' most-cited), cards stay compact — pill · title · chamber · status · Track — the detail lines would double the rail for secondary content. **Track button (roadmap preview, decided Jul 2026):** bill tracking is a not-yet-live roadmap feature, so the Track button renders as an inert, de-emphasized **dashed** preview control everywhere it appears (search-result cards, home, and these answer/rail cards) — white fill, dashed `rgba(17,21,15,0.3)` border, muted `#4f5651` label with a leading "+", hovering to `#11150f`. It is `aria-disabled`, does **nothing** on click (no sign-in, no state change, no page change), and has **no** affirmed "Tracking" state. It previews that tracking will exist without implying it works; the dashed treatment + the nav's roadmap entry carry that (no "coming soon" label). Implemented as the shared `RoadmapTrackButton` component. The live sign-in-to-track flow and the affirmed "✓ Tracking" state described below are **deferred until the tracking feature ships**.
...
- **Returning from sign-in:** authenticating from an answer page always returns to that exact answer (the `?q=` re-run) with the composer unlocked — never a generic dashboard, and never make the user redo the click that sent them to auth. (The clicked-intent-completed path for Track — affirmed "✓ Tracking" + a "Now tracking SF 2310." toast — is **deferred with the tracking feature**; see the Track button note above. Tracking is currently a roadmap preview, so Track is not an intent that can send a user to sign-in.)
