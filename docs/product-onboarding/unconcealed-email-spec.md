...
Close, Escape and outside press dismiss confirmation, with focus returned to invitation or heading after Google. Status messages are polite; errors are alerts. Touch targets are at least 44px; sheet buttons 54px. Long addresses wrap. Loading/ready/saving should hold space; uncertain-result text may add space. Once a save failure appears, retain its naturally wrapping space through retry and success so the action does not jump again; the reserved copy is hidden from assistive technology.
...
The reviewed keyboard drawing keeps the site’s 2px purple marker, 2px outside the control. Checkbox rows use a 10px corner radius and 12px horizontal padding offset by negative 12px margins, preserving label alignment while giving the marker room. Mouse and touch do not show that keyboard marker.
