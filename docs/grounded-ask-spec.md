...
| `topic_legislators` (`legislators`) | Fixed framing note (§4.3) directly under the H1; provenance strip; legislator rows grouped by chamber (name · party–district · "Authored N · Co-authored M" · View profile →) with an expandable "on the record" strip of the underlying bill pills; "See all N bills in Search →" overflow; Copy link/Share | Slim: most-cited bills as a Track roster. |
...
| **Matched-topic pill on `legislators`** — same interpreted-topic affordance `bills-list` has | [#85](https://github.com/alethical-org/alethical/issues/85) | Header already carries H1 + framing note + provenance; pill omitted to avoid stacking | Pure design pass; take it with the next Answer-page visual iteration. |
