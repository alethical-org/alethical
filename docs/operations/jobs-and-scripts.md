...
| Campaign money refresh (`.github/workflows/campaign-money-refresh.yml`) | Daily at 15:30 UTC, and by hand | Reads the Board's 6 registered-filer and current-report lists, refreshes the official totals for every supported year when a list changed or weekly, downloads the 3 payment files daily and publishes what passes every check, then clears saved pages and re-checks the published figures; opens or updates 1 issue when a step does not finish | No paid AI call; public downloads from cfb.mn.gov and the existing database |
...
The `scripts/` folder has 76 runnable files. GitHub jobs call 29 of them, and the
...
| Purpose | Files |
| --- | --- |
| Import official records or test data | `build_legislative_district_boundaries.py`, `build_zip_state_reference.py`, `load_campaign_finance.py`, `load_campaign_finance_filings.py`, `refresh_campaign_finance.py` (the daily campaign-money refresh, all steps in order), `load_lobbying.py` (paired current lobbyists and spending), `load_lobbying_expenditures.py` (spending-only compatibility command), `load_minnesota_data.py`, `load_refund_summaries.py`, `load_sample_data.py` |
