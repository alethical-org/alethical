...
| Configuration file | Root `railway.json` | Keeps Railway pointed at the reviewed build and release settings in this repository. | Unchecked: the trusted checker must land on `main` before this new Railway read can use the existing `RAILWAY_TOKEN`; [#1572](https://github.com/alethical-org/alethical/issues/1572) |
