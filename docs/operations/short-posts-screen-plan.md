...
The frontend suite covers 3,635 checks; after fixing the reduced-motion test setup, all affected tests pass. Type checking and the local production build pass. The first-download check measures 295,973 bytes against the 296,022-byte limit; the hosted build must meet the same limit with its real settings. Hosted checks and live routes remain the final release steps. No real Short post is in the public registry; the private trial, social posters and source files stay outside git.
