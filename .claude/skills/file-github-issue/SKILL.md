...
- **Milestone** — *which release/phase.* `v0 hardening` → `v1` → `v2` → `v8 candidates` → `v9 tbd` (the numeric `v#` prefix forces the list order; defer work by **moving an issue to a higher-numbered bucket**, never by renaming a milestone). Single source of truth for phase — no fractional milestones. Note rule 13: milestone is irrelevant to *staffing* — a later bucket is never a reason to leave goal-supporting work unowned.
- **Epic** (project single-select) — *which product area.* Grounded Ask & AI · Bills · Legislators · Votes & roll-calls · Candidates · Data & Ingestion · Platform & Ops · Tracking & Notifications · Home & Navigation · Notes & Pending (last = parked zero-risk items). **"Candidates" is the permanent name for this area — never "Elections"** (Eugene, Jul 26 2026; the milestone is likewise `v8 candidates`). If the board's option still reads "Candidates & Elections", set that existing value and leave the rename to Eugene: renaming a single-select option is `updateProjectV2Field`, which rule 4 reserves to him because it silently rearranges his grouped views. Platform items also take a **Platform Track** (CI & Testing / Backend architecture / Deploy & Ops / Security & auth).
...
**In a remote / web session, the board half is unreachable — plan for it rather than discovering it.** Claude Code on the web has no `gh` CLI, and its GitHub proxy serves **only a pinned set of PR-review GraphQL operations**; any other query returns *"This GraphQL query is not enabled for this session."* Since Projects v2 is GraphQL-only with no REST equivalent, **Epic, LOE, Status, and Platform Track cannot be set from such a session** (verified 2026-07-26 while filing #646/#647). What *is* reachable there: `--label` and `--milestone` equivalents, native **Type**, and the org-level issue fields **Priority** and **Effort** via the `issue_write` tool's `issue_fields` (list them with `list_issue_fields` — note these org issue-fields are a *different* mechanism from the project board's fields, and `Effort` here is the single-select High/Medium/Low, not the board's numeric `LOE`). So from a remote session: set everything you can, then **say plainly in your summary which board fields are still unset** — do not report the issue as fully categorized. The sweep command below is the backstop and will list them as `epic==null`, which is exactly what it exists for.
