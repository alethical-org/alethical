...
4. **`CONTRIBUTING.md` de-duplication**: three mirrored sections → pointers.
5. **`frontend-screen-system-design.md` restructure**: extract the still-live content rules (Bill Detail Content Rules · Global Empty/Loading/Error States · Cross-Platform Interaction Rules — none of which has another home), archive the 16-screen pre-build plan.
