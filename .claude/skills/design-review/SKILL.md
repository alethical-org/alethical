...
**0. Frame it.** Identify the page, the preview-band state(s) shown (reference frames only by Claude Design's own band labels — never invented names, per the `claude-design-prompt-rules` memory), its place in the IA and `docs/product-and-onboarding/mvp-redesign-plan.md`, and pull the governing spec (`docs/product-and-onboarding/product-scope.md`, `docs/product-and-onboarding/grounded-ask-spec.md`, the relevant issues/milestone). State in one line what this screen is and is for.
...
**6. Route the outcome.** Settled design decisions → `docs/product-and-onboarding/mvp-redesign-plan.md`. Capability/data gaps that need work → a GitHub issue filed at discovery (`.claude/rules/workflow.md` rule 4). Once the design finalizes, the build runs through `design-intake` → `design-build`.
...
`.claude/rules/grounded-answers.md` (the invariants step 1 enforces) · `docs/product-and-onboarding/mvp-redesign-plan.md` (decisions land here) · `docs/product-and-onboarding/grounded-ask-spec.md` · `docs/product-and-onboarding/product-scope.md` · `docs/design/ui-copy-guide.md` · the `claude-design-prompt-rules` and `eugene-workflow-preferences` memories · sibling skills `design-intake`, `design-build`, `design-audit`.
