...
**Tier the build before step 0.** Name the model, the tool's exact reasoning label and the mechanism for this build in the first reply (a `Tier:` line, per `~/.claude/CLAUDE.md`), before pinning the bundle or writing a line of code. A build is mixed work — bundle reading, file sweeps and test updates separate cleanly from layout and copy judgment — so the mechanism half is a real call here, not a silent default.
