          ...
          if (kind === 'list') {
            ...
            ready =
              ...
              visible(`a[href^="${committee}/payments?"]`).some(
                (node) =>
                  /^View receipts and expenditures$/.test(node.textContent || '') &&
...
async function waitForRequestedContent(page: Page, kind: string) {
  ...
  if (kind === 'list') {
    ...
    if (kind === 'payments') {
      ...
      await expect(
        page.getByRole('link', { name: /^View receipts and expenditures$/ }),
      ).toBeVisible();
    }
  }
...
test('committee to payments and Back preserves the requested year', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await page.getByRole('link', { name: /^View receipts and expenditures$/ }).click();
