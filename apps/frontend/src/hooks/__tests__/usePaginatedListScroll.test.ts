...
describe('computeScrollTarget', () => {
  // Reader is scrolled down (scrollTop 1000) and the list's top sits 900px below
  // the scroller's top. Landing it `offset` (20) below the top means scrolling to
  // 1000 + 900 - 20 = 1880.
  it('lands the list top `offset` px below the scroller top', () => {
  ...
  // List already at/above the viewport top: the naive target goes negative, so the
  // clamp keeps it at the start of the page instead of lurching backward.
  it('clamps at 0 when the list is already at/above the top', () => {
    expect(computeScrollTarget(5, 100, 100, 20)).toBe(0);
    expect(computeScrollTarget(0, -500, 100, 20)).toBe(0);
  });

  // List top is exactly `offset` below the scroller top already: no movement.
  it('does not move when the list is already at the target', () => {
