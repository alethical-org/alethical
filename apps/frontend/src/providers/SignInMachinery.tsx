...
import { PENDING_SIGN_IN_KEY, REQUESTED_SCREEN_KEY } from '../lib/auth/signInWorkPending';
import {
...
//
// This file is FETCHED WHEN SIGN-IN IS NEEDED, not with every page: it and the
// client behind it are ~260,000 minified bytes that a reader who is not signed in
// and is not signing in never touches (#1976). `SignInModalProvider.tsx` holds the
// seam — it hands every screen `openSignIn` from the first byte and fetches this
// when someone presses it, or when `signInWorkPendingOnLoad` says a saved session,
// a sign-in return or a stashed request means work is already waiting.

const isWeb = Platform.OS === 'web';
const PENDING_KEY = PENDING_SIGN_IN_KEY;
const OPEN_SIGN_IN_KEY = REQUESTED_SCREEN_KEY;
...
/**
 * Everything the sign-in dialog needs to run, mounted the moment sign-in is
 * reachable and never before.
 *
 * `onReady` hands the real `openSignIn` back up to `SignInModalProvider.tsx`,
 * which was standing in for it until this arrived. `initialRequest` is the press
 * that started the fetch, replayed here so a reader who clicked Track sees the
 * dialog they asked for rather than nothing.
 */
export function SignInMachinery({
  onReady,
  initialRequest,
}: {
  onReady: (openSignIn: (request: SignInRequest) => void) => void;
  initialRequest: SignInRequest | null;
}) {
  const {
    isLoading,
    isSignedIn,
  ...
  // Hand the real opener up, so a press now runs the flow rather than arming it.
  useEffect(() => {
    onReady(openSignIn);
  }, [onReady, openSignIn]);

  // The press that started the fetch, replayed once this is running.
  const replayed = useRef(false);
  useEffect(() => {
    if (replayed.current || !initialRequest) return;
    replayed.current = true;
    openSignIn(initialRequest);
  }, [initialRequest, openSignIn]);
