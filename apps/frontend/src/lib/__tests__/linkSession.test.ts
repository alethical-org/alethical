...
function lineageSession(userId: string, token: string, sessionId: string) {
  const payload = globalThis
    .btoa(JSON.stringify({ sub: userId, session_id: sessionId }))
    .replace(/=/g, '')
    .replace(/\+/g, '-')
    .replace(/\//g, '_');
  return {
...
describe('requested sign-in screen fallback', () => {
  it('lets expired links start Create or Recover while another account is open', () => {
    expect(canOpenRequestedSignInScreen('create', true)).toBe(true);
    expect(canOpenRequestedSignInScreen('recover', true)).toBe(true);
    expect(canOpenRequestedSignInScreen('sign-in', true)).toBe(false);
    expect(canOpenRequestedSignInScreen('sign-in', false)).toBe(true);
  });

  it('opens Create account from an expired confirmation link', () => {
    expect(requestedSignInState(null, '#auth_screen=create')).toEqual({
      screen: 'create',
      cleanHash: '',
    });
  });
...
describe('temporary session after a password save', () => {
  ...
  it('hands the verified session to the ordinary client when no account is open', () => {
    expect(
      decideTemporarySessionAfterPassword({
        ordinaryAccountId: null,
        ordinaryProviderUserId: null,
        temporaryAccountId: 'target-account',
        temporaryProviderUserId: 'target-provider',
      }),
    ).toEqual({
      relationship: 'none',
  ...
  it('replaces a matching ordinary session because the password save revoked it', () => {
    expect(
      decideTemporarySessionAfterPassword({
        ordinaryAccountId: 'shared-account',
        ordinaryProviderUserId: 'shared-provider',
        temporaryAccountId: 'shared-account',
        temporaryProviderUserId: 'shared-provider',
      }),
    ).toEqual({
      relationship: 'same',
      handToOrdinaryClient: true,
  ...
  it('keeps a valid provider session when 2 provider IDs open the same Alethical account', () => {
    expect(
      decideTemporarySessionAfterPassword({
        ordinaryAccountId: 'shared-account',
        ordinaryProviderUserId: 'google-provider',
        temporaryAccountId: 'shared-account',
        temporaryProviderUserId: 'password-provider',
      }),
    ).toEqual({
      relationship: 'same',
  ...
  it('clears only the temporary session when a different account is open', () => {
    expect(
      decideTemporarySessionAfterPassword({
        ordinaryAccountId: 'open-account',
        ordinaryProviderUserId: 'open-provider',
        temporaryAccountId: 'target-account',
        temporaryProviderUserId: 'target-provider',
      }),
    ).toEqual({
    ...
    expect(temporarySessionRelationship('open-account', 'target-account')).toBe('different');
  });

  it('hands no-account and same-account sessions to the ordinary client without revoking them', async () => {
    for (const ordinaryAccountId of [null, 'target-account']) {
      const ordinarySession = ordinaryAccountId
        ? {
            access_token: 'ordinary-access',
            refresh_token: 'ordinary-refresh',
            user: { id: 'target-provider' },
          }
        : null;
      const ordinary = {
        setSessionIfUnchanged: vi.fn(async () => ({
          changed: false,
          data: { session: null },
          error: null,
        })),
        clearSessionIfUnchanged: vi.fn(async () => true),
      };
      const temporary = {
      ...
      await expect(
        finishTemporarySessionAfterPassword({
          ordinary,
          ordinaryAccountId,
          ordinaryProviderUserId: ordinaryAccountId ? 'target-provider' : null,
          ordinarySession,
        ...
        error: null,
        relationship: ordinaryAccountId ? 'same' : 'none',
      });
      expect(ordinary.setSessionIfUnchanged).toHaveBeenCalledWith(ordinarySession, {
        access_token: 'temporary-access',
        refresh_token: 'temporary-refresh',
      });
  ...
  it('keeps the open session when a different provider ID reaches the same Alethical account', async () => {
    const ordinary = {
      setSessionIfUnchanged: vi.fn(async () => ({
        changed: false,
        data: { session: null },
    ...
    await expect(
      finishTemporarySessionAfterPassword({
        ...
        ordinarySession: {
          access_token: 'ordinary-access',
          refresh_token: 'ordinary-refresh',
          user: { id: 'google-provider' },
        },
        passwordChanged: true,
        session: {
          access_token: 'temporary-access',
          refresh_token: 'temporary-refresh',
          user: { id: 'password-provider' },
        },
        temporaryAccountId: 'shared-account',
        temporary,
      }),
    ).resolves.toEqual({ error: null, relationship: 'same' });
    expect(ordinary.setSessionIfUnchanged).not.toHaveBeenCalled();
  ...
  it('clears a revoked matching session when handing off the surviving session fails', async () => {
    ...
    const clearOrdinarySession = vi.fn(async () => true);
    const temporary = {
      signOut: vi.fn(async () => ({ error: null })),
    };
    ...
    await expect(
      finishTemporarySessionAfterPassword({
        ordinary: {
          ...
          clearSessionIfUnchanged: clearOrdinarySession,
        },
        ordinaryAccountId: 'target-account',
        ordinaryProviderUserId: 'target-provider',
        ordinarySession,
        passwordChanged: true,
        session: {
          access_token: 'temporary-access',
    ...
    expect(clearOrdinarySession).toHaveBeenCalledTimes(2);
    expect(temporary.signOut).not.toHaveBeenCalled();
  });
  ...
  it('preserves a fresh same-user session created after the password save', async () => {
    const before = lineageSession('target-provider', 'before', 'old-session');
    const after = lineageSession('target-provider', 'after', 'fresh-session');
    const ordinary = {
      setSessionIfUnchanged: vi.fn(async () => ({
        changed: false,
        data: { session: null },
    ...
    await expect(
      finishTemporarySessionAfterPassword({
        ...
        ordinaryAccountId: 'target-account',
        ordinaryProviderUserId: 'target-provider',
        ordinarySession: after,
        ordinarySessionAtPasswordMutation: before,
        passwordChanged: true,
        session: lineageSession('target-provider', 'temporary', 'temporary-session'),
        temporaryAccountId: 'target-account',
        temporary,
      }),
    ).resolves.toEqual({ error: null, relationship: 'same' });
