 ...
 * (GET /campaign-finance/races; issue #1954). Ordered by office, district, then
 * filed name, and by nothing else: the endpoint declares no sort parameter, so
 * none is offered here.
 */
export async function getCampaignFinanceRacesFromApi(options: {
  year: number;
  office?: string;
}): Promise<MoneyByRacePage> {
  const params = new URLSearchParams({ year: String(options.year) });
  if (options.office) params.set('office', options.office);
  const response = await publicApiRequest<DetailResponse<ApiMoneyByRacePayload>>(
    `/campaign-finance/races?${params.toString()}`,
  );
  return getCampaignFinanceRacesFromApiPayload(response.data, options.year);
}

export interface ApiNameSearchRowPayload {
  kind?: string;
  id?: string | null;
  slug?: string | null;
